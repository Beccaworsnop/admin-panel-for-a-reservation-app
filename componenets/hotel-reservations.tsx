...
import { NewHotelReservationModal } from "./new-hotel-reservation-modal"
...
const hotelReservations = [
  ...
  },
  {
    ...
    supplierInfo: {
      bookedThrough: "Direct",
      supplierName: "Hotel Direct",
...
export function HotelReservations() {
  ...
  const applyFilters = () => {
    let filtered = hotelReservations
  ...
  const handleExport = () => {
    ...
    const csvData = [
      ...
      ...filteredReservations.map((res) => [
        ...
        res.bookingSource,
        res.membershipLevel,
        res.supplierInfo.bookedThrough,
        res.supplierInfo.supplierName,
        res.supplierInfo.creationDate,
        res.supplierInfo.state,
        res.managedBy || "",
    ...
    <div className="p-4 lg:p-6">
      ...
      <Card className="p-4 mb-6">
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3 lg:gap-4">
          <div className="flex-1 relative">
            ...
            <Input
              placeholder="Search hotel reservations by name or ID..."
              className="pl-10"
              value={searchQuery}
      ...
      <Card>
        <div className="p-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">Hotel Reservations ({filteredReservations.length})</h3>
        </div>
            ...
            <div key={reservation.id} className="p-4 border-b border-gray-200 last:border-b-0">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <Badge variant="outline" className="text-xs">
                    {reservation.subStatus === "Available" ? "Disponible" : "Sur Demande"}
                  </Badge>
                </div>
              </div>
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900">${reservation.salePrice}</span>
                </div>
                {reservation.managedBy && (
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Managed By:</span>
                    <span className="text-sm text-gray-900">{reservation.managedBy}</span>
                  </div>
                )}
              </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredReservations.map((reservation) => (
                <tr key={reservation.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">${reservation.salePrice}</div>
                    <div className="text-xs text-gray-500">Purchase: ${reservation.purchasePrice}</div>
                    <div className="text-xs text-green-600">Margin: ${reservation.margin}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{reservation.managedBy || "Not assigned"}</div>
                    {reservation.managerResponsible && (
                      <div className="text-xs text-gray-500">Manager: {reservation.managerResponsible}</div>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <Button
      ...
      <NewHotelReservationModal isOpen={showNewReservationModal} onClose={() => setShowNewReservationModal(false)} />
    </div>
  )
}
